...
const ChkrDay: <TMessage extends IMessage = IMessage>(p: ChkrDayProps<TMessage>)=> React.ReactElement | null = props => {
  const { currentMessage, previousMessage, dateFormat = DATE_FORMAT, dateTextStyle, nextMessage, renderDay, inverted } = props

  if ((!inverted && currentMessage && !isSameDay(currentMessage, previousMessage!))
