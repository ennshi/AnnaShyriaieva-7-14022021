...
const ChkrMessage: <TMessage extends IMessage = IMessage>(p: ChkrMessageProps<TMessage>)=> React.ReactElement = props => {
  const { currentMessage, showUserAvatar = true, position, previousMessage, nextMessage, renderAvatar, inverted, renderBubble } = props
  ...
  return (
    <Box w="100%" mt={(!inverted && !previousMessage?.text) || (inverted && !nextMessage?.text) ? 'auto' : '0'}>
      <ChkrDay {...props} />
      <HStack
        spacing="8px"
        w="100%"
        alignItems="flex-end"
        mt={(!inverted && isSameDay(currentMessage!, previousMessage!)
          && (isSameUser(currentMessage!, previousMessage) ? '8px' : '24px')) || 0}
        mb={(inverted && isSameDay(currentMessage!, previousMessage!)
            && (isSameUser(currentMessage!, previousMessage) ? '8px' : '24px')) || 0}
      >
