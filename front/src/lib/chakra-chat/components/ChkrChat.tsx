...
const ChkrChat: <TMessage extends IMessage = IMessage>(p: ChkrChatProps<TMessage>) => React.ReactElement = props => {
  const {
    infiniteScroll = true,
    inverted,
    messages,
  ...
  return (
    <>
      <Flex
        ...
        {...chatWrapperStyle}
      >
        <ScrollContextProvider messages={messages} refContainer={containerRef} inverted={inverted}>
          {renderChkrHeader()}
          <Flex
            w="100%"
            flex="auto"
