...
export const ScrollContextProvider: React.FC<ScrollProviderType> = ({
  messages,
  refContainer,
  inverted,
  children,
  ...
  const onScrollToStart = useCallback(() => {
    const scrollToBottom = () => {
      if (
        ...
        refContainer.current.scrollHeight >
          refContainer.current.scrollTop + refContainer.current.offsetHeight
      )
        refContainer.current.scrollTop =
          refContainer.current.scrollHeight - refContainer.current.offsetHeight;
    };
    ...
    if (inverted) return scrollToTop();
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      if (!refContainer?.current) return;
      if (startAutoScrollToStart()) return setShouldScrollToStart(true);
  ...
  const onScrollForFewPixels = (px: number) => {
    ...
    if (refContainer?.current) {
      if (shouldScrollToStart) setTimeout(onScrollToStart, 50);
  ...
  return (
    <ScrollContext.Provider value={{ onScrollToStart, onScrollForFewPixels }}>
      {children}
...
export const useScroll = () => {
  const { onScrollToStart, onScrollForFewPixels } =
    useContext<ScrollContextType>(ScrollContext);
