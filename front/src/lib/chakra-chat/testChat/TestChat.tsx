...
const TestChat: React.FC<BoxProps> = (props) => {
  ...
  useEffect(() => {
    if (newMessage?.text)
      setMessages((prevState) =>
        prevState.length ? [...prevState, newMessage] : [newMessage]
      );
    ...
    return new Promise<CustomMessage[]>((res) => {
      setTimeout(() => {
        const newMessages = [...DEFAULT_MESSAGES]
          ...
          .slice(page.current * limitItems, (page.current + 1) * limitItems);
  ...
  const handleLoadMoreItems = async () => {
    ...
    if (!cancelFetch.current) {
      setLoading(false);
      page.current += 1;
      if (loadedMessages.length < limitItems) setHasNextPage(false);
      if (messages) {
  ...
  const renderBubble = (properties: ChkrBubbleProps<CustomMessage>) => {
    const { currentMessage } = properties;
    ...
    if (isCustom) {
      const renderInnerText = (text: string): React.ReactNode => {
        ...
        return innerText.map(
          (part: [text: string, style: TextProps], i: number) => (
            <Text key={i} {...part[1]}>
              {part[0]}
            </Text>
          )
      ...
      return <ChkrBubble renderInnerText={renderInnerText} {...properties} />;
    }

    return <ChkrBubble {...properties} />;
  };
