...
const TestChatNoInfiniteScrollInverse: React.FC<BoxProps> = props => {
  const [messages, setMessages] = useState<IMessage[]>(DEFAULT_MESSAGES.sort((a: any, b: any) => b.createdAt - a.createdAt))
  const [newMessage, setNewMessage] = useState<IMessage>()

  useEffect(() => {
    if (newMessage?.text)
      setMessages(prevState => (prevState.length ? [newMessage, ...prevState] : [newMessage]))
